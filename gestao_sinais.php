...
// Proteção de Acesso (Mesma do Dashboard)
...
// Verifica se é Admin ou Supervisor para acessar esta página
...
<head>
    ...
    <style>
        :root {
            ...
            --text-main: #f0f6fc; --text-dim: #8b949e; --vip: #ffd700; --danger: #ff4d4d;
        }
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Inter', sans-serif; }
        ...
        .nav-btn { 
            ...
            display: flex; align-items: center; gap: 12px; font-size: 13px; font-weight: 500; transition: 0.3s; margin-bottom: 2px;
        }
        ...
        .perf-header { font-size: 11px; font-weight: 800; text-transform: uppercase; margin-bottom: 20px; display: flex; align-items: center; gap: 8px; color: var(--text-dim); }
        .perf-stats-row { display: grid; grid-template-columns: repeat(4, 1fr); text-align: center; }
        .stat-box span { display: block; font-size: 9px; color: var(--text-dim); margin-bottom: 5px; }
        ...
        .tag-status { padding: 4px 10px; border-radius: 6px; font-size: 10px; font-weight: 800; border: 1px solid transparent; }
...
<main>
    <div style="margin-bottom: 30px;">
        <h1 style="font-size: 24px; font-weight: 800;">Gestão de Sinais</h1>
        <p style="color: var(--text-dim); font-size: 14px;">Administre os palpites em tempo real para os usuários.</p>
    </div>

    <!-- PERFORMANCE REAL -->
    <div class="perf-grid">
        <div class="perf-card">
            <div class="perf-header"><i class="fas fa-chart-line"></i> STATUS GRÁTIS</div>
        ...
        <div class="perf-card" style="border-color: var(--vip);">
            <div class="perf-header" style="color: var(--vip);"><i class="fas fa-gem"></i> STATUS VIP</div>
            <div class="perf-stats-row">
    ...
    <div class="table-wrapper">
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td style="color: var(--primary); font-family: monospace;"><?= $s['p_codigo'] ?></td>
                    <td><b style="color: <?= ($s['p_categoria'] == 'VIP') ? 'var(--vip)' : 'var(--text-dim)' ?>"><?= strtoupper($s['p_categoria']) ?></b></td>
                    <td><?= $s['p_confronto'] ?></td>
                    <td style="font-weight: 700;"><?= $s['p_placar'] ?: '- x -' ?></td>
                    <td><?= $s['p_mercado'] ?></td>
                    ...
                    <td style="text-align:right">
                        <a href="status.php?id=<?= $s['id'] ?>&set=Green" title="Green"><i class="fas fa-check-circle" style="color: var(--primary); margin-left: 10px;"></i></a>
                        <a href="status.php?id=<?= $s['id'] ?>&set=Red" title="Red"><i class="fas fa-times-circle" style="color: var(--danger); margin-left: 10px;"></i></a>
                        <a href="apagar.php?id=<?= $s['id'] ?>" title="Excluir"><i class="fas fa-trash" style="color: var(--text-dim); margin-left: 10px;"></i></a>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
