...
// 3. Filtro de Busca e Paginação (Mantidos)
...
$sql_membros = "SELECT id, nome, login, email, creditos, perfil FROM usuarios" . $where_clause . " ORDER BY id DESC LIMIT ? OFFSET ?";
...
<head>
    ...
    <style>
        :root {
            --primary: #00ff88; --bg: #0d1117; --card: #161b22; --border: #30363d;
            --text-main: #f0f6fc; --text-dim: #8b949e; --vip: #ffd700; --danger: #ff4d4d; --info: #3498db;
        }
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Inter', sans-serif; }
        body { background-color: var(--bg); color: var(--text-main); display: flex; min-height: 100vh; }

        /* SIDEBAR COMPLETA MANTIDA */
        nav { 
            width: 280px; background: rgba(22, 27, 34, 0.95); backdrop-filter: blur(10px);
            border-right: 1px solid var(--border); padding: 30px 15px;
            display: flex; flex-direction: column; position: fixed; height: 100vh; overflow-y: auto; z-index: 1000;
        }
        ...
        .nav-label { font-size: 10px; color: var(--text-dim); text-transform: uppercase; letter-spacing: 1px; margin: 15px 0 8px 15px; display: block; font-weight: 700; }
        ...
        .btn-pub { background: var(--primary); color: #0d1117; border: none; padding: 12px 25px; border-radius: 12px; font-weight: 800; cursor: pointer; text-transform: uppercase; }
        ...
        .btn-action { background: none; border: none; cursor: pointer; font-size: 16px; transition: 0.2s; padding: 5px; }
        ...
        #modalEditar {
            ...
            background:rgba(0,0,0,0.8); z-index:3000; align-items:center; justify-content:center; backdrop-filter: blur(8px);
        }
    </style>
</head>
<body>
    ...
    <section class="search-container">
        <form method="GET" class="search-grid">
            <div class="input-group">
                ...
                <input type="text" name="search" placeholder="Nome, e-mail ou login..." value="<?= htmlspecialchars($search) ?>">
            </div>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><code><?= htmlspecialchars($m['login']) ?></code></td>
...
<!-- MODAL EDITAR -->
<div id="modalEditar">
    <div style="background:var(--card); width:95%; max-width:550px; padding:30px; border-radius:24px; border:1px solid var(--border);">
        ...
        <form action="gestao_membros.php" method="POST">
            ...
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:15px;">
                ...
                <div class="input-group"><label>Senha (vazio p/ manter)</label><input type="text" name="senha" placeholder="Nova senha..."></div>
...
function confirmarExclusao(nome) { return confirm("TEM CERTEZA QUE DESEJA EXCLUIR DEFINITIVAMENTE O USUÁRIO: " + nome + "?"); }
